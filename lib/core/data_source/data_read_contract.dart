/// Describes where a source-neutral read result came from.
enum DataReadOrigin {
  network,
  freshSnapshot,
...
/// Describes how current the returned data is known to be.
enum DataReadFreshness { current, freshCache, staleOrUnknown }

/// A capability must never be inferred from an empty nullable field.
...
final class DataReadMetadata {
  ...
  DataReadMetadata merge(DataReadMetadata other) {
    return DataReadMetadata(
  ...
  static DataReadFreshness _weakerFreshness(
    ...
    return _freshnessRank(left) >= _freshnessRank(right) ? left : right;
  }

  static int _freshnessRank(DataReadFreshness value) {
    return switch (value) {
...
/// Immutable capability declaration with fail-closed intersection semantics.
final class DataCapabilitySet<T extends Enum> {
  ...
  factory DataCapabilitySet.supported(Iterable<T> capabilities) {
    return DataCapabilitySet<T>(<T, DataCapabilitySupport>{
  ...
  DataCapabilitySupport supportOf(T capability) {
    return _values[capability] ?? DataCapabilitySupport.unknown;
  }

  bool supports(T capability) {
    return supportOf(capability) == DataCapabilitySupport.supported;
  }
