...
final class DefaultNovelReaderHtmlPreparationService
  ...
  Future<NovelReaderPreparedChapter> prepare({
    ...
    final project = _chapterProjection(
      episode.episodeId,
      flowUnitExtractor,
      semanticDocument,
    );
    ...
    if (source is NovelHtmlChapterRenderPreparer) {
      return source.prepareAndProject(
        rawHtml: rawHtml,
    ...
    return NovelReaderBackgroundWork.run(
      codeUnits: prepared.document.preparedHtml.length,
      transform: () => project(prepared),
    );
  }
}

NovelReaderPreparedChapter Function(NovelHtmlPreparedChapter)
_chapterProjection(
  String episodeId,
  ...
  return NovelReaderPreparedChapter(
    ...
    scrollFragments: NovelReaderScrollMarkup.fragments(scrollHtml),
    renderDocument: prepared.document,
    flowUnits: flowUnits,
