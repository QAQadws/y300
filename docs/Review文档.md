...
- [ ] `docs/收藏漫画联动与图片解析分阶段重构方案.md`
- [ ] `docs/开发文档.md`
- [ ] `docs/Review文档.md`

## Review 重点

- [ ] 方案明确指出 `FavoriteSyncService` 的远端同步、详情解析、模块入库、漫画刷新、重复合并、快照写出和刷新通知职责过宽。
- [ ] 方案没有建议一次性重写，而是按阶段迁移收藏详情解析 loader、内容处理器 registry、模块后处理任务和漫画刷新结果应用器。
- [ ] 方案覆盖收藏页、漫画页、小说页之间通过刷新总线和任务进度联动的现状与改造方向。
- [ ] 方案覆盖收藏页解析行为，包括 tag 查询、内容分类、漫画/小说/普通帖子分发。
- [ ] 方案覆盖不同图片 URL 来源，包括 DOM 图片、attachment 图片、伪绝对附件 URL、第三方图片和缓存/展示 normalize。
- [ ] 方案包含优先级、阶段验收标准和 Review 清单。
- [ ] 本轮仅文档变更，未执行测试、分析或格式化。

---

# API 登录与登出迁移方案 Review 清单（2026-05-29）
...
- [ ] 方案明确当前实现是 Web 登录，目标是移动端 API 登录/登出。
- [ ] 方案没有把登录业务继续耦合到 `ApiClient`，而是拆分 formhash 获取、API 请求、会话验证和 repository 编排。
- [ ] 方案说明登录前游客态也可获取 `formhash`，且获取和提交必须复用同一 CookieStore。
- [ ] 方案说明 Cookie 是后续 API 和受保护图片请求的鉴权来源。
- [ ] 方案说明图片请求只带 headers，不在请求体中携带 `formhash`。
- [ ] 方案包含 API 登出后清理本地 Cookie 的策略，并区分服务端登出失败与仅本地清除。
- [ ] 方案包含安全问答、验证码、错误处理和测试计划。
- [ ] 本轮仅文档变更，未执行测试、分析或格式化。
