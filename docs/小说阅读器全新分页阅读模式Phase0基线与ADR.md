...
> 后续状态：Phase 3 已解除运行时强制纵向降级并启用 HTML-first 分页；Phase 4 已按 ADR-004 完成 v34 阅读位置身份持久化；Phase 5 已完成锚点交互、有限页面挂载、布局诊断和长章节响应性收口；Phase 6 已完成代码收口和自动化验收，真机矩阵仍需人工执行。本文件保留 Phase 0 的历史边界，后续实现以各阶段实际交付记录为准。
