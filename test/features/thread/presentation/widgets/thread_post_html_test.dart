...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        cachedImage.request?.sourceUrl,
        'https://bbs.yamibo.com/data/attachment/forum/page-1.jpg',
      );
      expect(cachedImage.request?.role, ImageCacheRole.threadInline);
      expect(
        await cachedImage.headerBuilder?.buildHeaders(
          cachedImage.request!.sourceUrl,
        ),
        <String, String>{
          'Referer': 'https://bbs.yamibo.com/',
  ...
  testWidgets('ThreadPostHtml renders smiley gif as inline image', (
    ...
    final cachedImage = tester.widget<CachedLibraryImage>(inlineImages);
    expect(
      await cachedImage.headerBuilder?.buildHeaders(
        cachedImage.request!.sourceUrl,
      ),
      <String, String>{'Referer': 'https://bbs.yamibo.com/'},
    );
    expect(
