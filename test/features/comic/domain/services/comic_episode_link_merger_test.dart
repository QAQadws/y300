...
void main() {
  group('DefaultComicEpisodeLinkMerger', () {
    ...
    test('fromSearchCandidates excludes current tid and extracts episode label', () {
      ...
      expect(
        links.map((link) => link.episodeTitle).toList(),
        <String>['第81话', '第82话上', '第82话下'],
      );
    });
    ...
    test('sort keeps numbered chapters before unnumbered extras', () {
      ...
      expect(
        sorted.map((link) => link.rawText).toList(),
        <String>['第1话', '第2话上', '第2话下', '番外'],
      );
    });
  });
}
