...
void main() {
  ...
  group('ComicFavoriteAutoRefreshCoordinator title resolution', () {
    test(
      'catalog hit delegates refresh application with catalog source',
      () async {
        ...
        expect(
          refreshService.catalogRequests.single.displayTitle,
          'Catalog Comic',
        );
        expect(refreshService.catalogPreloadedRootDetails.single?.tid, '100');
        ...
        expect(
          ...
          ComicEpisodeRefreshSource.catalog,
        );
        expect(
    ...
    test('catalog direct fast path when catalogUrl is provided', () async {
      ...
      expect(
        applier.requests.single.reason,
        'favorite_comic_catalog_direct_refresh',
      );
    ...
    test(
      ...
      () async {
        ...
        expect(
          catalogUrlUpdater.updates.single.catalogUrl,
    ...
    test(
      'catalog miss uses cleaned favorite title for queue and parsed source title for search',
      () async {
        final refreshService = _FakeRefreshService(
          catalogOutcome: const ComicEpisodeRefreshOutcome(
            source: ComicEpisodeRefreshSource.empty,
        ...
        expect(
          ...
          'favorite_comic_search_refresh_queued',
        );
    ...
    test(
      'catalog miss skips search queue when tag is not long-running',
      () async {
        final refreshService = _FakeRefreshService(
          catalogOutcome: const ComicEpisodeRefreshOutcome(
            source: ComicEpisodeRefreshSource.empty,
    ...
    test(
      'catalog miss queues search for long-running type id without tag name',
      () async {
        final refreshService = _FakeRefreshService(
          catalogOutcome: const ComicEpisodeRefreshOutcome(
            source: ComicEpisodeRefreshSource.empty,
        ...
        final result = await coordinator.refreshAfterFavoriteIngest(
          comicId: 'comic:69',
          detail: _detail(
            subject: '[Scan] Long Running Comic EP 01',
            typeid: '69',
          ),
          favoriteTitle: 'Long Running Favorite',
          sourceTagName: null,
        );

        expect(result.status, ComicFavoriteAutoRefreshStatus.queuedForSearch);
        expect(searchQueue.enqueuedRequests, hasLength(1));
        expect(searchQueue.enqueuedRequests.single.comicId, 'comic:69');
        expect(
          bus.signal.value?.reason,
          'favorite_comic_search_refresh_queued',
        );
    ...
    test(
      'forced catalog miss cleans favorite title for queue and parses fallback search request',
      () async {
        final refreshService = _FakeRefreshService(
          catalogOutcome: const ComicEpisodeRefreshOutcome(
            source: ComicEpisodeRefreshSource.empty,
        ...
        expect(result.status, ComicFavoriteAutoRefreshStatus.queuedForSearch);
        expect(searchQueue.enqueuedTitles, <String>['Raw Queue Title']);
    ...
    test(
      'bootstrap initial catalog miss enqueues to search queue (no inline governed search)',
      () async {
...
class _FakeRefreshService implements ComicEpisodeRefreshService {
  ...
  final List<ComicEpisodeRefreshRequest> searchRequests =
      <ComicEpisodeRefreshRequest>[];
  final List<ComicThreadDiscoveryDocument?> catalogPreloadedRootDetails =
      <ComicThreadDiscoveryDocument?>[];
  final List<ComicThreadDiscoveryDocument?> searchPreloadedRootDetails =
      <ComicThreadDiscoveryDocument?>[];
  int directCalls = 0;
...
class _RecordingSearchQueue implements ComicSearchRefreshQueueEnqueuer {
  ...
  final List<ComicThreadDiscoveryDocument?> enqueuedPreloadedDetails =
      <ComicThreadDiscoveryDocument?>[];
...
class _CatalogUrlUpdate {
  const _CatalogUrlUpdate({required this.comicId, required this.catalogUrl});
