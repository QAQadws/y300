...
void main() {
  ...
  test(
    'loadChrome requests mobile index once with mobile user agent',
    () async {
      ...
      expect(
        result.dataOrNull!.carouselItems.single.aspectRatio,
        closeTo(3.0, 0.01),
      );
  ...
  test(
    'loadChrome returns empty carousel without fallback when mobile index is empty',
    () async {
      final adapter = _ForumHomeChromeTestAdapter(emptyMobileIndex: true);
      final repository = _buildRepository(adapter);
...
class _ForumHomeChromeTestAdapter implements HttpClientAdapter {
  _ForumHomeChromeTestAdapter({this.emptyMobileIndex = false});
