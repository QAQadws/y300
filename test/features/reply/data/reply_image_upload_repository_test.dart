...
void main() {
  group('DiscuzReplyImageUploadRepository', () {
    ...
    test('prepareUpload fails when attach remain is exhausted', () async {
      final repository = _buildRepository(
        remoteDataSource: _FakeUploadRemoteDataSource(
          permission: _permission(
            attachRemain: const ReplyAttachRemain(size: -1, count: 0),
          ),
        ),
      );
    ...
    test('uploadImage returns uploaded image when remote returns positive aid', () async {
      final fileSystem = _fileSystemWithFile('/gallery/photo.jpg');
      final remoteDataSource = _FakeUploadRemoteDataSource(
        uploadResponse: const ReplyImageUploadResponse(
          aid: '123456',
          rawBody: '123456',
          statusCode: 200,
        ),
    ...
    test('uploadImage fails when remote aid is negative non-number or empty', () async {
      for (final rawAid in <String>['-1', 'not-aid', '']) {
        final fileSystem = _fileSystemWithFile('/gallery/photo.jpg');
        final repository = _buildRepository(
          remoteDataSource: _FakeUploadRemoteDataSource(
            uploadResponse: ReplyImageUploadResponse(
              aid: rawAid,
              rawBody: rawAid,
              statusCode: 200,
            ),
...
DiscuzReplyImageUploadRepository _buildRepository({
...
ReplyImageUploadPermission _permission({
  String uid = '597454',
  String uploadHash = 'upload-hash',
  Set<String> allowedExtensions = const {'jpg', 'jpeg', 'png', 'gif'},
  ReplyAttachRemain attachRemain = const ReplyAttachRemain(size: -1, count: -1),
}) {
...
class _FakeUploadRemoteDataSource
  ...
  _FakeUploadRemoteDataSource({
    ReplyImageUploadPermission? permission,
    this.uploadResponse = const ReplyImageUploadResponse(
      aid: '123456',
      rawBody: '123456',
      statusCode: 200,
    ),
  ...
  final ReplyImageUploadPermission permission;
  final ReplyImageUploadResponse uploadResponse;
  final Object? uploadException;
  final List<ReplyLocalImageFile> uploadedFiles = <ReplyLocalImageFile>[];
