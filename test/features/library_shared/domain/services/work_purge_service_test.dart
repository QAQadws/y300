...
void main() {
  group('DefaultWorkPurgeService', () {
    test(
      'comic purge clears core collaborators before best-effort cleanup',
      () async {
        final calls = <String>[];
        final service = DefaultWorkPurgeService(
          ...
          purgeLibraryWorkState: ({required moduleKey, required workId}) async {
          ...
          deleteCacheByOwner: ({required ownerType, required ownerId}) async {
    ...
    test(
      'novel purge uses novel strategy and can return empty best-effort flags',
      () async {
        final calls = <String>[];
        final service = DefaultWorkPurgeService(
          purgeComicWork: ({required comicId}) async {
    ...
    test(
      'cache and download failures are reported without rolling back core cleanup',
      () async {
        final calls = <String>[];
        final service = DefaultWorkPurgeService(
          purgeComicWork: ({required comicId}) async {
