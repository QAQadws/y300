...
void main() {
  group('FlutterLocalLibraryTaskNotificationService', () {
    ...
    test(
      'shows a determinate progress notification when total is provided',
      () async {
        final client = _FakeNotificationClient();
        final service = FlutterLocalLibraryTaskNotificationService(
          client: client,
    ...
    test(
      'shows an indeterminate progress notification without a total',
      () async {
        final client = _FakeNotificationClient();
        final service = FlutterLocalLibraryTaskNotificationService(
          client: client,
    ...
    test(
      'does not show and does not throw when permission is denied',
      () async {
        final client = _FakeNotificationClient(
          permission: LibraryTaskNotificationPermissionState.denied,
        );
    ...
    test(
      'clear cancels the matching notification id after initialize',
      () async {
        final client = _FakeNotificationClient();
        final service = FlutterLocalLibraryTaskNotificationService(
          client: client,
    ...
    test(
      'heartbeat re-posts active notification to keep the timeout alive',
      () {
        fakeAsync((async) {
          final client = _FakeNotificationClient();
          final service = FlutterLocalLibraryTaskNotificationService(
          ...
          expect(client.shownRequests.every((r) => r.id == 3002), isTrue);
        });
      },
    );
